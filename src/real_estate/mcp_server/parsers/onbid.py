...
def _onbid_extract_items(
    ...
    if response:
        ...
        # 차세대 온비드(2026~)는 header/body를 최상위에 둔다. 오류는 result 래퍼.
        top_header = _as_str_key_dict(payload.get("header"))
        top_body = _as_str_key_dict(payload.get("body"))
        top_result = _as_str_key_dict(payload.get("result"))
        if top_header or top_body:
            header = top_header or payload
            body = top_body or payload
        elif top_result:
            header = top_result
            body = top_result
        else:
